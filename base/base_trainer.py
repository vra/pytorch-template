...
class BaseTrainer:
    ...
    def train(self):
        ...
        Full training logic
        """
        time_from_very_begin = time.time()
        time_curr_epoch_start = time.time()
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            for key, value in result.items():
                if key == 'metrics':
                    log.update({mtr.__name__: value[i] for i, mtr in
                                enumerate(self.metrics)})
                elif key == 'val_metrics':
                    log.update({'val_' + mtr.__name__: value[i] for i, mtr in
                                enumerate(self.metrics)})
                else:
                    log[key] = value
            ...
            if self.mnt_mode != 'off':
                try:
                    ...
                    self.logger.warning(
                        "Warning: Metric '{}' is not found. Model performance",
                ...
                if not_improved_count > self.early_stop:
                    ...
                    break
            time_curr_epoch_passed = time.time() - time_curr_epoch_start
            time_passed = time.time() - time_from_very_begin
            print('Time of current epoch: {:.2f}s. Time from begining: {:.2f}s'
                  .format(time_curr_epoch_passed, time_passed))
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        self.logger.info("Checkpoint '{}' (epoch {}) loaded"
